...
class RaceParams(object):
    ...
    def randomiseArchetypePreference():
        for r in RaceParams.RACES:
            ...
            new_hl = rng.choice(list(set(LocationParams.ARCHETYPES) - set(new_pl)), int(1 + rng.random() * 2),
                                replace=False)
            ...
            print(
                f"{r.name} prefs in {[LocationParams.ARCHETYPES_STR[a] for a in new_pl]} and hated is {[LocationParams.ARCHETYPES_STR[a] for a in new_hl]}")
...
def map_coord_to_screen_coord_centered(map_coord):
    ceiled_tw = math.ceil(UserInterfaceParams.SCREENSIZE[
                  0] * UserInterfaceParams.GRAPH_SURFACE_WIDTH_PROPORTION / ModelParams.MAP_SIZE)
