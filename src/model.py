...
class Map(object):
    ...
    def generate_quadmap(self):
        ...
        def split(r):
            ...
            r4 = Rect((math.floor(r.size[0] / 2), math.floor(r.size[1] / 2)),
                      (math.ceil(r.size[0] / 2), math.ceil(r.size[1] / 2)))
...
class BuildingFactory(object):
    ...
    def _build(building, comm):
        ...
        try:
            _r = (comm.buildings[building.name][0] + 1,
                  {"value": value, "effect_name": building.effect_name, "is_percent": ispercent})
        except KeyError:
...
class Community(object):
    ...
    def update_actual_prod_and_store(self):
        for r in params.ModelParams.RESSOURCES:
            self.actual_production[r] = (self.location.base_production[r] + sum(
                self.ressource_production_flat_bonus[r].values())) * (
                                                    1.0 + sum(self.ressource_production_percent_bonus[r].values()))
            self.actual_storage[r] = self.location.base_storage[r] * (
                        1.0 + sum(self.ressource_storage_percent_bonus[r].values()))

    def update_production_bonus(self):
        for r in self.ressource_production_flat_bonus.keys():
            self.ressource_production_flat_bonus[r]["POP"] = self.get_total_pop() / 100.0 * \
                                                             self.location.bonus_per_100_pop[r]
    ...
    def update_pops(self):
        ...
        drf_wealth = 1 - (self.ressource_stockpile[params.ModelParams.WEALTH] / self.actual_storage[
            params.ModelParams.WEALTH])
        ...
        for race in self.population.keys():
            ...
            for social_class in self.population[race].keys():
                ...
                self.population[race][social_class] = float(self.population[race][social_class]) + (
                            (float(self.population[race][social_class]) * (_race_class_growth_rate / 100.0)) / 4.0)

            # self.race_growth_rate[race] = (_race_birth_rate - _race_death_rate)
            self.race_growth_rate[race] = ((_avg_birth_rate_race / len(self.population[race].keys())) - (
                        _avg_death_rate_race / len(self.population[race].keys())))
    ...
    def caravan(self):
        if self.model != None:
            ...
            self.randomness_of_life["birth_rate_factor"][_race] += abs(
                self.randomness_of_life["birth_rate_factor"][_race] * 0.2)
            ...
            random_proportion_poor_class = 1.0 - (
                        random_proportion_middle_class + random_proportion_bourgeoisie_class + random_proportion_noble_class)
            ...
            self.log_event(
                f"A caravan of {_pop_number} {_race.name} arrived ({int(noble_pop)} {params.SocialClassParams.NOBILITY.adjective_short}, {int(bourgeois_pop)} {params.SocialClassParams.BOURGEOISIE.adjective_short}, {int(middle_pop)} {params.SocialClassParams.MIDDLE.adjective_short} and {int(poor_pop)} {params.SocialClassParams.POOR.adjective_short}).",
                f"A caravan of {_pop_number} {_race.name} arrived in {self.name}.")
    ...
    def update_social_indexes(self):
        try:
            ...
            wealth_factor = self.ressource_stockpile[params.ModelParams.WEALTH] / self.actual_storage[
                params.ModelParams.WEALTH]

            self.social_ascension_index = (1.0 * ((1.0 - nobility_prop) * wealth_factor)) + self.actual_production[
                params.ModelParams.WEALTH]

            poor_prop = (self.get_total_pop_class(params.SocialClassParams.POOR) / self.get_total_pop())

            self.social_decay_index = 1.0 + (poor_prop - nobility_prop) + (1.0 - wealth_factor) + sum(
                list(self.effective_consumption[params.ModelParams.WEALTH].values()))
        except ZeroDivisionError:
    ...
    def social_evolution(self):
        ...
        for _race in params.RaceParams.RACES:
            if self.get_total_pop_race(_race) > 0:
                ...
                _poors_ascend = (self.population[_race][params.SocialClassParams.POOR] * (
                            (self.social_ascension_index * ascend_random_factor) / 100.0))

                _middle_ascend = (self.population[_race][params.SocialClassParams.MIDDLE] * (
                            (self.social_ascension_index * ascend_random_factor) / 100.0))
                _middle_descent = (self.population[_race][params.SocialClassParams.MIDDLE] * (
                            (self.social_decay_index * descend_random_factor) / 100.0))

                _bourgeois_ascend = (self.population[_race][params.SocialClassParams.BOURGEOISIE] * (
                            (self.social_ascension_index * ascend_random_factor) / 100.0))
                _bourgeois_descent = (self.population[_race][params.SocialClassParams.BOURGEOISIE] * (
                            (self.social_decay_index * descend_random_factor) / 100.0))

                _nobles_descend = self.population[_race][params.SocialClassParams.NOBILITY] * (
                            (self.social_decay_index * descend_random_factor) / 100.0)

                self.population[_race][params.SocialClassParams.POOR] = max(0, self.population[_race][
                    params.SocialClassParams.POOR] + math.floor(_middle_descent - _poors_ascend))
                self.population[_race][params.SocialClassParams.MIDDLE] = max(0, self.population[_race][
                    params.SocialClassParams.MIDDLE] + math.floor(
                    (_poors_ascend + _bourgeois_descent) - (_middle_ascend + _middle_descent)))
                self.population[_race][params.SocialClassParams.BOURGEOISIE] = max(0, self.population[_race][
                    params.SocialClassParams.BOURGEOISIE] + math.floor(
                    (_middle_ascend + _nobles_descend) - (_bourgeois_ascend + _bourgeois_descent)))
                self.population[_race][params.SocialClassParams.NOBILITY] = max(0, self.population[_race][
                    params.SocialClassParams.NOBILITY] + math.floor(_bourgeois_ascend - _nobles_descend))
    ...
    def a_day_passed(self):
        ...
        self.effective_consumption[params.ModelParams.FOOD]["POP"] = self.food_consumption_from_pop * (
                    1.0 + sum(self.consumption_reduction[params.ModelParams.FOOD].values()))
        ...
        for r in set(params.ModelParams.RESSOURCES) - set([params.ModelParams.WEALTH]):
            self.ressource_stockpile[r] = utils.clamp(
                self.ressource_stockpile[r] + sum(self.effective_gain[r].values()) - sum(
                    self.effective_consumption[r].values()), 0, self.actual_storage[r])
            if self.ressource_stockpile[r] >= self.actual_storage[r]:
                if r != params.ModelParams.FOOD:
                    self.effective_gain[params.ModelParams.WEALTH]["SURPLUS_SELL"] = sum(
                        self.effective_gain[r].values()) - sum(
                        self.effective_consumption[r].values()) * self.get_trade_factor()
        ...
        fcpp_new_increased_value = min(self.food_consumption_per_pop + self.food_consumption_per_pop_step,
                                       self.food_consumption_per_pop_max)
        fcpp_new_decreased_value = max(self.food_consumption_per_pop - self.food_consumption_per_pop_step,
                                       self.food_consumption_per_pop_min)
        ...
        if (sum(self.effective_gain[params.ModelParams.FOOD].values()) - sum(
                self.effective_consumption[params.ModelParams.FOOD].values())) > 0:
        ...
        # Calculate Wealth
        self.ressource_stockpile[params.ModelParams.WEALTH] = utils.clamp(
            self.ressource_stockpile[params.ModelParams.WEALTH] + sum(
                self.effective_gain[params.ModelParams.WEALTH].values()) - sum(
                self.effective_consumption[params.ModelParams.WEALTH].values()), 0,
            self.actual_storage[params.ModelParams.WEALTH])
        ...
        stored_food_unrest = (
                    self.ressource_stockpile[params.ModelParams.FOOD] - self.actual_storage[params.ModelParams.FOOD])
        ...
        for r in params.ModelParams.RESSOURCES:
            self.ressources_prev_net_worth[r] = sum(self.effective_gain[r].values()) - sum(
                self.effective_consumption[r].values())
    ...
    def a_month_passed(self):
        for race in params.RaceParams.RACES:
            ...
            self.randomness_of_life["birth_rate_factor"][race] += ((-0.03 + params.rng.random() * 0.06) + (
                        0.03 * main_race_influence)) / 3.0
            self.randomness_of_life["death_rate_factor"][race] += ((-0.03 + params.rng.random() * 0.06) - (
                        0.00 * main_race_influence)) / 3.0

            self.randomness_of_life["birth_rate_factor"][race] = utils.clamp(
                self.randomness_of_life["birth_rate_factor"][race], 0, 0.2)
            self.randomness_of_life["death_rate_factor"][race] = utils.clamp(
                self.randomness_of_life["death_rate_factor"][race], 0, 0.2)
    ...
    def to_string(self):
        ...
        s += "Happiness : {:.2f}; Growth rate : {:.2f}; Space : {:.2f}/{:.2f}\n".format(self.happiness,
                                                                                        self.net_growth_rate,
                                                                                        self.space_used,
                                                                                        self.location.space)
        for r in self.ressource_stockpile:
            s += "{:.0f}/{:.0f} (+{:.3f}; {:.3f}+{:.3f}-{:.3f})\n".format(self.ressource_stockpile[r],
                                                                          self.actual_storage[r],
                                                                          sum(self.effective_gain[r].values()) - sum(
                                                                              self.effective_consumption[r].values()),
                                                                          self.location.base_production[r],
                                                                          sum(self.ressource_production_flat_bonus[r]),
                                                                          sum(self.effective_consumption[r].values()))
    ...
    def to_string_list(self, used_font=None):
        ...
        lines.append("{} in {} ({}, {:.02f})".format(self.name, self.location.name,
                                                     params.LocationParams.ARCHETYPES_STR[
                                                         self.location.archetype].title(),
                                                     self.location.base_attractiveness))
        lines.append(
            f"{'▼' if self.show_kingdom else '►'} {self.kingdom.name} (K to {'hide' if self.show_kingdom else 'show'})")
        if self.show_kingdom:
            ...
            lines.append(
                f"{tab}a{'' if self.kingdom.main_race.name_adjective.lower()[0] in ['d', 'h'] else 'n'} {self.kingdom.main_race.name_adjective} {params.KingdomParams.POLITICS_STR[self.kingdom.main_politic]} and {params.KingdomParams.POLITICS_STR[self.kingdom.secondary_politic]} {self.kingdom.governement_name_simple}")
            ...
            for k in self.kingdom.relations:
                lines.append(
                    f"{tab}{'▼' if self.show_kingdom_details else '►'} {k.name}: {sum(self.kingdom.relations[k].values())}")
        ...
        if self.show_pop_details:
            ...
            if used_font != None:
                _class_race_offset_pxvalue = (
                    max([used_font.size(_class.name)[0] for _class in params.SocialClassParams.CLASSES]))
                _class_race_offset = ""
                while used_font.size(_class_race_offset)[0] < _class_race_offset_pxvalue:
                    _class_race_offset += " "

                _race_header = f"{tab}{_class_race_offset}  " + str(
                    [_r.name for _r in params.RaceParams.RACES] + ['Total']).replace('\'', '').replace("[", '').replace(
                    "]", '').replace(",", " |")
                _race_value = f"{tab}{_class_race_offset} " + str([
                                                                      f"{int(self.get_total_pop_race(_r))}{get_str_offset(str(int(self.get_total_pop_race(_r))), [_r.name], used_font)}"
                                                                      for _r in params.RaceParams.RACES] + [
                                                                      self.get_total_pop()]).replace('\'', '').replace(
                    "[", '').replace("]", '').replace(",", " |")

                _class_prop = []
                for _sc in params.SocialClassParams.CLASSES:
                    _class_name_offset = get_str_offset(_sc.name, [_c.name for _c in params.SocialClassParams.CLASSES],
                                                        used_font)
            ...
            lines.append(
                f"{tab}Social ascension: {self.social_ascension_index:.2f}, Social decay: {self.social_decay_index:.2f}")

            lines.append(
                "{}Basic growth rate: {:+.2f} ({:.2f}-{:.2f})".format(tab, self.net_growth_rate, self.actual_birth_rate,
                                                                      self.actual_death_rate))
            s = ""
            for race in params.RaceParams.RACES:
                try:
        ...
        if self.show_ressources:
            ...
            lines.append(
                f"{tab}Trade factor: {self.get_trade_factor():0.3f} (Own TF: {self.trade_factor:0.3f}, Neighbours TF: {self.get_trade_factor_from_neighours():0.3f})")
            for r in self.ressource_stockpile:
                try:
                    lines.append("{}{} : {:.0f}/{:.0f} ({:+.3f}; {:.3f}+{:.3f}-{:.3f})".format(tab,
                                                                                               params.ModelParams.RESSOURCES_STR[
                                                                                                   r].lower().title(),
                                                                                               self.ressource_stockpile[
                                                                                                   r],
                                                                                               self.actual_storage[r],
                                                                                               sum(self.effective_gain[
                                                                                                       r].values()) - sum(
                                                                                                   self.effective_consumption[
                                                                                                       r].values()),
                                                                                               self.location.base_production[
                                                                                                   r], sum(
                            self.ressource_production_flat_bonus[r].values()), sum(
                            self.effective_consumption[r].values())))
                except KeyError:
                    pass
        else:
        ...
        lines.append(
            f"{'▼' if self.show_buildings else '►'} Buildings (B to {'hide' if self.show_buildings else 'show'})")
        if self.show_buildings:
            # buildings[BUILDING_NAME] = (LEVEL, {"value":TOTAL_EFFECT, "effect_name":NAME})
            for k in self.buildings:
                if self.buildings[k][1]['is_percent']:
                    lines.append(
                        f"{tab}{k} lvl {self.buildings[k][0]}, {self.buildings[k][1]['effect_name']}: {(self.buildings[k][1]['value']) * 100:+.2f}%")
                else:
                    lines.append(
                        f"{tab}{k} lvl {self.buildings[k][0]}, {self.buildings[k][1]['effect_name']}: {(self.buildings[k][1]['value']):+.3f}")
        ...
        if len(self.location.landmarks) > 0:
            if self.show_landmarks:
                lines.append("▼ Landmarks, happiness mod. {:+.02f}. (L to hide)".format(
                    sum([lm.happiness_value for lm in self.location.landmarks])))
                for lm in sorted(self.location.landmarks, key=lambda x: x.happiness_value, reverse=True):
                    lines.append(tab + lm.to_string())
            else:
                lines.append("► Landmarks, happiness mod. {:+.02f}. (L to show)".format(
                    sum([lm.happiness_value for lm in self.location.landmarks])))

        nb_to_show = 5
        if self.show_events:
            lines.append(
                f"▼ Event log, entr{'y' if len(self.event_log.get_lines()) <= 1 else 'ies'} {max(0 if len(self.event_log.get_lines()) == 0 else 1, len(self.event_log.get_lines()) - nb_to_show + 1)} to {len(self.event_log.get_lines())}. (O to {'hide' if self.show_events else 'show'})")
            for l in self.event_log.get_lines()[-nb_to_show:]:
                lines.append(f"{tab}{l}")
        else:
            lines.append(
                f"► Event log, {len(self.event_log.get_lines())} entr{'y' if len(self.event_log.get_lines()) <= 1 else 'ies'}. (O to {'hide' if self.show_events else 'show'})")
...
class AIKingdomController(object):
    ...
    def __init__(self, diplomatic_stance):
        ...
        self.agriculture_building_action = [BuildingFactory.build_barn, BuildingFactory.build_farm,
                                            BuildingFactory.build_granary]
    ...
    def take_action_tmp(self, comm):
        actions = params.rng.choice([a[0] for a in self.action_list],
                                    p=[a[1] / sum([x[1] for x in self.action_list]) for a in self.action_list])
    ...
    def develop_agriculture(self, comm):
        action = params.rng.choice(self.agriculture_building_action,
                                   p=[x / sum(self.agriculture_buildings_importance) for x in
                                      self.agriculture_buildings_importance])
        action(comm)
...
class Kingdom(object):
    ...
    def init_gov_and_politics(self):
        ...
        try:
            self.gov_type_str = params.rng.choice(
                params.KingdomParams.GOVERNEMENT_TYPE_STR[(self.main_politic, self.secondary_politic)])
            self.governement = params.rng.choice(
                params.KingdomParams.GOVERNEMENT_FROM_POLICIES[(self.main_politic, self.secondary_politic)])
        except KeyError:
            self.gov_type_str = params.rng.choice(
                params.KingdomParams.GOVERNEMENT_TYPE_STR[(self.secondary_politic, self.main_politic)])
            self.governement = params.rng.choice(
                params.KingdomParams.GOVERNEMENT_FROM_POLICIES[(self.secondary_politic, self.main_politic)])

        self.governement_name = params.rng.choice(params.KingdomParams.GOVERNEMENTS_STR[self.governement])
        self.governement_name_simple = params.KingdomParams.GOVERNEMENTS_STR_SIMPLE[self.governement]
        self.name = self.name.upper().replace("GOVNAME", params.rng.choice(
            params.KingdomParams.GOVERNEMENTS_STR[self.governement])).title()
    ...
    def generate_name(self):
        ...
        govadj = params.rng.choice(["", params.rng.choice(
            [self.main_race.name_adjective, params.rng.choice(params.KingdomParams.GLOBAL_KINGDOM_ADJ_ONLY),
             f"{self.main_race.name_adjective} {params.rng.choice(params.KingdomParams.GLOBAL_KINGDOM_ADJ_ONLY)}"],
            p=[0.3, 0.55, 0.15])], p=[0.5, 0.5])

        govname = params.rng.choice([params.rng.choice(self.communities).location.name,
                                     params.rng.choice(params.KingdomParams.GLOBAL_KINGDOM_NAMES_ONLY)], p=[0.3, 0.7])
        try:
    ...
    def monthly_update(self):
        ...
        for k in nkingdom:
            ...
            self.relations[k]["Aff.MP"] = params.KingdomParams.POLITICS_AFFINITY[self.main_politic][k.main_politic] + (
                        params.KingdomParams.POLITICS_AFFINITY[self.main_politic][k.secondary_politic] / 2.0)
            self.relations[k]["Aff.SP"] = (params.KingdomParams.POLITICS_AFFINITY[self.secondary_politic][
                                               k.main_politic] / 2.0) + (
                                                      params.KingdomParams.POLITICS_AFFINITY[self.secondary_politic][
                                                          k.secondary_politic] / 4.0)
...
class Model(object):
    ...
    def random_model_map_basic(self, scale):
        ...
        while _continue and len(chosen_positions) < self.max_location:
            ...
            _max_try = 100
            while ((
                           flatten_tiles[random_i].type in [params.TileParams.WATER, params.TileParams.DEEPWATER,
                                                            params.TileParams.PEAKS]
        ...
        for i, p in enumerate(chosen_positions):
            ...
            location = Location(archetype,
                                location_names[i][:-1] if location_names[i][-1] == '\n' else location_names[i])
        ...
        kingdom_colors = params.rng.choice(list(params.UserInterfaceParams.COLOR_LIST.keys()), len(self.locations),
                                           replace=False)
    ...
    def to_string_summary(self, used_font=None):
        ...
        lines.append(
            f"{'▼' if self.show_race else '►'} Races information (R to {'hide' if self.show_race else 'show'})")
        if self.show_race:
            s1 = "     "
            s2 = "     "
            for i, r in enumerate(params.RaceParams.RACES):
            ...
            for r in params.RaceParams.RACES:
                ...
                offset_pref = get_str_offset(
                    str([params.LocationParams.ARCHETYPES_STR[a] for a in r.preferred_locations]),
                    [str([params.LocationParams.ARCHETYPES_STR[a] for a in r.preferred_locations]) for r in
                     params.RaceParams.RACES], used_font)
                lines.append(
                    f"     {r.name}{offset_name} prefs are {[params.LocationParams.ARCHETYPES_STR[a] for a in r.preferred_locations]}{offset_pref} and hated are {[params.LocationParams.ARCHETYPES_STR[a] for a in r.hated_locations]}")

        k_list = self.get_kingdoms()

        lines.append(
            f"{'▼' if self.show_population else '►'} Population: {sum_all_pop} inhabitants in {len(k_list)} Kingdom{'s' if len(k_list) > 1 else ''} (P to {'hide' if self.show_population else 'show'})")
        if self.show_population:
            str_popprop = "     "
            for race in sorted(_total_pop, key=_total_pop.get, reverse=True):
                if _total_pop[race] != 0:
                    str_popprop += "{:.0f}% {} ({:.1f}k), ".format((_total_pop[race] / sum_all_pop) * 100, race.name,
                                                                   _total_pop[race] / 1000)
        ...
        lines.append(
            f"{'▼' if self.show_happiness else '►'} Average happiness: {sum([c.happiness for c in self.communities]) / len(self.communities):.2f} (H to {'hide' if self.show_happiness else 'show'})")
        ...
        if gen_roads:
            ...
            for n in g.nodes:
                ...
                for e in n.edges:
                    ...
                    _end = self.map.get_tile(e.end.info["location"].map_position[0],
                                             e.end.info["location"].map_position[1])
